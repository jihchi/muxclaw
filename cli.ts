...
async function storeLoad(
	...
	let prompt: string;
	try {
		prompt = (await Deno.readTextFile(join(messageDir, 'prompt.txt')))
			.trim();
	} catch {
		throw new Error(`message not found: ${channel}:${id}`);
	}

	const metaPath = join(messageDir, 'meta.json');
	let meta: JobMeta;
	try {
		meta = v.parse(
			JobMeta,
